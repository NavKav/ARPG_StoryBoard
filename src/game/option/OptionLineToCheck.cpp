...
void OptionLineToCheck::display(int x, int y) {
    ...
    _window.writeText(x, y, "Fullscreen: ");// écrit "Fullscreen" aux coordonnées (x,y) avec la couleur/police/taille sélectionnées
    int w = 0, h = 0;
    _window.textSizeOf("Fullscreen",w,h);
    if (_checked == false) {
        _window.scaleIMG(x + w + 30, y, 25,25,"/option/uncheckedbox.png");
    }
    else {
